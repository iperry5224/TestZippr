...
class NISTApplicationController:
    ...
    def render_assessment_page(self, region, selected_families, run_assessment, include_security_hub=False):
        ...
        if st.session_state.results and st.session_state.assessor:
            ...
            # Chad (AI) positioned above results, aligned right
            from pathlib import Path
            chad_avatar_path = Path(__file__).parent / 'assets' / 'chad2.jpg'
            
            # Determine risk level for Chad (AI)'s message
            ...
            # Build Chad (AI) avatar HTML (160px - compact for results page)
            ...
            # Create columns to position Chad (AI) with inline chat
            chad_spacer, chad_col = st.columns([1, 3])
            with chad_col:
                # Chad (AI) card container (compact - ~60% smaller)
                st.markdown(f"""
                ...
                # Inline conversational chat with Chad (AI)
                st.markdown("<div style='margin-top: 0.5rem;'></div>", unsafe_allow_html=True)
                
                # Initialize inline chat history
                ...
                with st.expander("💬 **Chat with Chad (AI)**", expanded=True):
                    ...
                    # Chat history display (height shrunk ~75% from 400 to 100)
                    ...
                    # Process the query if exists
                    if st.session_state.get('chad_inline_query'):
                        query = st.session_state.chad_inline_query
                        st.session_state.chad_inline_query = None  # Clear after use
                        
                        # Add user message to history
                        st.session_state.chad_inline_history.append({"role": "user", "content": query})
                        
                        # Build context and call Bedrock AI (no API key needed - uses AWS credentials)
    ...
    def render_chad_security_analyst_page(self):
        ...
        # Create header with avatar - using container for better layout
        st.markdown("""
        <style>
        .chad-header {
            display: flex;
        ...
        if chad_avatar_path.exists():
            ...
            st.markdown("""
            <div class="chad-header">
                <div style="width: 120px; height: 120px; background: linear-gradient(135deg, #3b82f6 0%, #1e3a5f 100%); 
                            border-radius: 12px; display: flex; align-items: center; justify-content: center;
                            font-size: 3.5rem; border: 3px solid #3b82f6; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3);">
                    🛡️
                </div>
                <div class="chad-info">
                    <h1>Chad (AI)</h1>
