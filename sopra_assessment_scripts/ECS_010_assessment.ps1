# SOPRA Assessment Script: ECS-010 - External Email Marking
# Category: Email & Communications Security
# Run on target machine; outputs CSV row for SOPRA import
# Correlates to remediation script: ECS_010.ps1
...
$category = "Email & Communications Security"
$control_id = "ECS-010"
$control_name = "External Email Marking"

# Discovery only - manual review required
try {
    $out = Invoke-Expression 'New-TransportRule -Name ''External Email Banner'' -FromScope NotInOrganization -PrependSubject ''[EXTERNAL] '''
