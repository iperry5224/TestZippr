# SOPRA Assessment Script: CRY-002 - TLS Configuration
# Category: Cryptographic Controls
# Run on target machine; outputs CSV row for SOPRA import
# Correlates to remediation script: CRY_002.ps1
...
try {
    $out = Invoke-Expression 'Set-ItemProperty -Path ''HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Server'' -Name Enabled -Value 0'
