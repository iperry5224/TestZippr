# SOPRA Assessment Script: CRY-010 - Random Number Generation
# Category: Cryptographic Controls
# Run on target machine; outputs CSV row for SOPRA import
# Correlates to remediation script: CRY_010.ps1
...
$category = "Cryptographic Controls"
$control_id = "CRY-010"
$control_name = "Random Number Generation"

# Discovery only - manual review required
try {
    $out = Invoke-Expression 'Set-ItemProperty -Path ''HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\FipsAlgorithmPolicy'' -Name ''Enabled'' -Value 1'
