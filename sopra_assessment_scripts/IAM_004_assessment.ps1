# SOPRA Assessment Script: IAM-004 - Account Lifecycle Management
# Category: Identity & Access Management
# Run on target machine; outputs CSV row for SOPRA import
# Correlates to remediation script: IAM_004.ps1
...
$category = "Identity & Access Management"
$control_id = "IAM-004"
$control_name = "Account Lifecycle Management"

# Discovery only - manual review required
try {
    $out = Invoke-Expression 'Get-ADUser -Filter * -Properties LastLogonDate | Where-Object {$_.LastLogonDate -lt (Get-Date).AddDays(-90)}'
