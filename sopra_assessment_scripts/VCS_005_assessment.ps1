# SOPRA Assessment Script: VCS-005 - VM Snapshot Management
# Category: Virtualization & Container Security
# Run on target machine; outputs CSV row for SOPRA import
# Correlates to remediation script: VCS_005.ps1
...
$category = "Virtualization & Container Security"
$control_id = "VCS-005"
$control_name = "VM Snapshot Management"

# Discovery only - manual review required
try {
    $out = Invoke-Expression 'Get-VM | Get-Snapshot | Where-Object {$_.Created -lt (Get-Date).AddHours(-72)} | Select-Object VM, Name, Created'
